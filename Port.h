 ...
 * for example Ahmed Bakr's ID = 1000 :) */
...
typedef uint8 Port_PinModeType;  /* used inside setPinMode function */
...
typedef struct
{
    ...
    Port_PinDirectionType direction;	/* PORT_PIN_IN, PORT_PIN_OUT  mandatory	in dio 				*/
    Port_InternalResistor resistor;		/* OFF,PULL_UP,PULL_DOWN 					*/
    Port_PinLevelValue initial_value;	/* PORT_PIN_LEVEL_LOW,PORT_PIN_LEVEL_HIGH mandatory	in dio	*/
    Port_PinInitialMode initial_mode;	/* e.g. PORT_PIN_MODE_DIO mandatory unless dio				*/
...
************************************************************************************/
void Port_Init( const Port_ConfigType* ConfigPtr );  /* set output to a default level before setting the port pin direction */
